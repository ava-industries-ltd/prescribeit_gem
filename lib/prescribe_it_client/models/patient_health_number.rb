  ...
  class PatientHealthNumber
    ...
    def self.openapi_types
      {
        ...
        :'identifier_type' => :'SharedPatientJurisdictionalHealthIdentifierTypeHolder'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PrescribeItClient::PatientHealthNumber` initialize method"
      end

      attributes = attributes.each_with_object({}) { |(k, v), h|
        symbolized_key = k.to_sym
        unless self.class.acceptable_attributes.include?(symbolized_key)
          fail ArgumentError, "`#{k}` is not a valid attribute in `PrescribeItClient::PatientHealthNumber`. Please check the name to make sure it's valid. List of attributes: " + self.class.acceptable_attributes.inspect
        end
        h[symbolized_key] = v
      }
    ...
    def hash
      [value, version_number, identifier_type].hash
    end
