...
module PrescribeItClient
  class OrganizationElasticSearchParameters
    attr_accessor :role
    ...
    def self.openapi_types
      {
        ...
        :'attribute_set' => :'OrganizationElasticSearchParametersAttributeSet',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PrescribeItClient::OrganizationElasticSearchParameters` initialize method"
      end

      attributes = attributes.each_with_object({}) { |(k, v), h|
        symbolized_key = k.to_sym
        unless self.class.acceptable_attributes.include?(symbolized_key)
          fail ArgumentError, "`#{k}` is not a valid attribute in `PrescribeItClient::OrganizationElasticSearchParameters`. Please check the name to make sure it's valid. List of attributes: " + self.class.acceptable_attributes.inspect
        end
        h[symbolized_key] = v
      }
    ...
    def hash
      [role, address_state_exact, search_string, attribute_set, entity_service_code, include_all_practitioners].hash
    end
