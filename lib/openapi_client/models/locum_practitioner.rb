...
module OpenapiClient
  class LocumPractitioner < CommunicationEntity
    ...
    def self.openapi_types
      {
        :'acting_on_behalf_of' => :'Practitioner',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::LocumPractitioner` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::LocumPractitioner`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [acting_on_behalf_of, license_number, contact_list, first_name, middle_name, last_name, name_prefix, name_suffix].hash
    end
