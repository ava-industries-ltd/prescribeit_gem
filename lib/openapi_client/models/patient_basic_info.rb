  ...
  class PatientBasicInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PatientBasicInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PatientBasicInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@other_names.nil? && @other_names.length > 2
        invalid_properties.push('invalid value for "other_names", number of items must be less than or equal to 2.')
      end
    ...
    def valid?
      ...
      return false if !@other_names.nil? && @other_names.length > 2
      true
    end
    ...
    def other_names=(other_names)
      ...
      if other_names.length > 2
        fail ArgumentError, 'invalid value for "other_names", number of items must be less than or equal to 2.'
      end
    ...
    def hash
      [first_name, middle_name, last_name, name_prefix, name_suffix, birth_date, jurisdictional_health_number, other_names].hash
    end
